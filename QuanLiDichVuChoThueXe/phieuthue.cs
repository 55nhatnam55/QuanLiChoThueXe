...
namespace QuanLiDichVuChoThueXe
{
    public partial class phieuthue : Form
    {
      ...
      private string kiemtra(string a)
        ...
        private string kiemtrabanglaiKH(string Makh)
        {
            tbl_KHACHHANG kh = connectdb.tbl_KHACHHANG.Single(e => e.MaKH.Equals(txtmkh.Text.Trim()));
            if (kh.BangLai != null)
                return kh.BangLai;
            return null;
        }
        ...
        private void btnSua_Click(object sender, EventArgs e)
        {
            ...
            {
                string Maphieu = dataGridView1.CurrentRow.Cells[0].Value.ToString();
                tbl_PHIEUTHUE pt = connectdb.tbl_PHIEUTHUE.Single(p => p.MaPhieu.Equals(Maphieu));
                pt.SoXe = txtsoxe.Text.Trim();
                pt.MaKH = txtmkh.Text.Trim();
                pt.NgayLapPhieu = DateTime.Parse(txtnlp.Text);
                pt.SoNgayMuon =int.Parse(txtsnm.Text.Trim());
                connectdb.SaveChanges();
                loaddb();
